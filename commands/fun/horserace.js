...
if(lang === 'en') {
    ...
    if(lang === 'it') {
              const chosenHorses = shuffle(horses).slice(0, 6);
        await message.channel.send(stripIndents`
			**Scegli un cavallo su cui scommettere:** _(Scrivi il numero)_
			${chosenHorses.map((horse, i) => `**${i + 1}.** ${horse.name}`).join('\n')}
		`);
        const filter = res => {
            if (res.author.id !== message.author.id) return false;
        ...
        if (!messages.size) return message.channel.send('Scusa, non puoi gareggiare senza scommettere!');
        const pick = chosenHorses[Number.parseInt(messages.first().content, 10) - 1];
        let results = [];
        for (const horse of chosenHorses) {
            results.push({
        ...
        const win = results[0].name === pick.name;
        db.add(`games_${message.author.id}`, 1)
        return message.channel.send(win ? `Ben fatto! Il tuo cavallo ha vinto!` : 'Buona fortuna per la prossima volta!', { files: [leaderboard] });
