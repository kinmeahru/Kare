...
if(lang === 'de') {
    ...
    if (!message.member.hasPermission("MANAGE_CHANNELS"))
    return message.channel.send({embed : {
			description : `${errorr} Du hast dafür keine Rechte!`,
			color : error_color
		}});
    if (message.member.hasPermission("MANAGE_CHANNEL"))
      if (isNaN(amount))
        return message.channel.send({embed : {
					description : `${error} Bitte geben Sie eine gültige Nummer ein!`,
					color : error_color
				}});
    if (args[0] === amount + "s") {
      message.channel.setRateLimitPerUser(amount);
				 if(amount > 21600) return message.channel.send({embed : {
		 description : `${error} Das Maximum an Sekunden beträgt 21.600!`,
		 color : error_color
	 }})

      else if (amount > 1) {
        message.channel.send({embed : {
					description : `${success} Der Slow-Modus ist jetzt ${amount} Sekunden`,
					color : success_color
				}});
        return;
      } else {
        message.channel.send({embed : {
					description : `${success} oder Slowmode ist jetzt ${amount} Sekunde`,
					color : success_color
				}});
        return;
      }//
    ...
    if (args[0] === amount + "m") {
      message.channel.setRateLimitPerUser(amount * 60);
				 if(amount > 360) return message.channel.send({embed : {
		 description : `${error} Das Maximum an Minuten beträgt 360 Minuten!`,
		 color : error_color
	 }})

      else if (amount > 1) {
        message.channel.send({embed : {
					description : `${success} Der Slow-Modus beträgt jetzt ${amount} Minuten`,
					color : success_color
				}});
        return;
      } else {
        message.channel.send({embed : {
					description : `${success} Der Slow-Modus ist jetzt ${amount} Minute`,
					color : success_color
				}});
    ...
    if (args[0] === amount + "h") {
      message.channel.setRateLimitPerUser(amount * 60 * 60);
	 if(amount > 6) return message.channel.send({embed : {
		 description : `${error} Das Maximum beträgt 6 Stunden!`,
		 color : error_color
	 }})
        ...
        message.channel.send({embed : {
					description : `${success} Der Slow-Modus beträgt jetzt ${amount} Stunden`,
					color : success_color
				}});
        return;		 
      } else {
      ...
      message.channel.send({embed : {
					description : `${error} Sie können nur Stunden(h), Minuten(m) und Sekunden(s) einstellen.`,
					color : error_color
				}});
    } 
